...
**TableScreen subclasses** use `OnShow` lifecycle (patch `OnShow`). **KModalScreen subclasses** use `Show` lifecycle (patch `Show`; do NOT patch `OnShow` — `KModalScreen.OnActivate` calls `OnShow(true)` directly during prefab init).
...
- `KModalScreen.OnActivate()` calls `OnShow(true)` directly during prefab initialization. Patching `OnShow` on KModalScreen subclasses triggers before the game world is ready. **Patch `Show()` instead.**
