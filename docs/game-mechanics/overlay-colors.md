...
Every overlay in ONI shows a legend panel on the right side of the screen via `OverlayLegend`. This document lists the actual color values used by each overlay. All colors are RGBA 0-255. Where alpha is 255 (fully opaque) it is omitted for brevity. Derived from decompiled source code (`OverlayLegend`, `OverlayModes`, `GlobalAssets.Instance.colorSet`).
...
| Entry | Color (RGBA) | Tooltip |
|-------|-------------|---------|
| Receptacle | white | Receives material from a Conveyor Rail and stores it for Receptacles |
...
No legend entries. Uses a filter menu to select element categories (Metal, Buildable, Liquid, Gas, etc.). Each tile is colored by its element's `substance.uiColour`. See `elements-solid.md`, `elements-liquid.md`, `elements-gas.md` for element properties. Element overlay colors are listed below.
...
Programmatically generated from `Db.Get().RoomTypes`, sorted by `sortKey`. Colors come from `RoomTypeCategories` via `colorSet.GetColorByName(category.colorName)`. See `rooms.md` for room detection mechanics, criteria, and bonuses.
...
## Element Overlay Colors

All `substance.uiColour` values used by the Materials overlay.
