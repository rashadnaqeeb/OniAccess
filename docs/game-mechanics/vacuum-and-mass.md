...
Exception: during save deserialization, if the cell is still solid and contains the same element, the entombed mass is absorbed into the cell via `AddRemoveSubstance()` instead of spawning a chunk.

**Minimum mass for absorption:** The chunk's mass must exceed ~0.01 kg for it to be absorbed into the cell. Below this threshold, the chunk is stored in the entombed items list instead (preserving its mass, temperature, and disease data separately from the cell).
...
1. **None** (AddRemoveSubstance) - Adds or removes mass from a cell. Intended to be clamped to maxMass, but see the clamping bug above.
