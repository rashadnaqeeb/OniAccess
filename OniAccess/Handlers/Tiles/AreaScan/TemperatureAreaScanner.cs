...
namespace OniAccess.Handlers.Tiles.AreaScan {
	public class TemperatureAreaScanner: IAreaScanner {
		public string Scan(int[] cells, int totalCells, int unexploredCount) {
			try {
				...
				for (int i = 0; i < cells.Length; i++)
					sum += Grid.Temperature[cells[i]];
				float avg = sum / cells.Length;

				tokens.Add(string.Format(
					STRINGS.ONIACCESS.BIG_CURSOR.AVG_TEMPERATURE,
					GameUtil.GetFormattedTemperature(avg)));
				return string.Join(", ", tokens);
			} catch (Exception ex) {
				Util.Log.Error($"TemperatureAreaScanner.Scan: {ex}");
