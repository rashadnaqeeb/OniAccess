...
c. **Traits** (Labels, one per trait): Access the trait entries from the CharacterContainer. Per research Finding 6: traits are dynamically created LocText elements. For each trait, build the full text per decision: "name, effect, and description all spoken together" -- "Mole Hands, +2 Digging, Moves through tiles faster". The trait LocText has the visible name+effect; the ToolTip has the full description. So: `GetWidgetSpeechText` for trait Labels returns the LocText.text (name+effect). Shift+I reads the ToolTip description (detailed explanation).

   Actually, re-reading the decision: "Trait readout: full info upfront without extra key: name, effect, and description all spoken together." This means the description should be in the normal readout, not just on Shift+I. So: build the label text as "name, effect, description" by combining the LocText visible text with the ToolTip description text. If ToolTip text is too long, truncate to first sentence.
...
- When Enter is pressed on a colony in the colony list view: the game expands to show individual saves. Override `ActivateCurrentWidget` to:
  ...
  2. Set `_inColonySaveView = true`
  3. Rediscover widgets (now shows individual saves)
  4. Speak: colony name then first save entry
